...
export default async function EditArtistPage({ params }: ArtistPageProps) {
  ...
  return <><AdminHeader admin={admin} /><main className="mx-auto max-w-6xl px-6 py-10 lg:px-10"><Link href="/admin/artists" className="font-bold underline">← Artists</Link><h1 className="mb-8 mt-4 text-4xl font-black">Edit {artist.name}</h1><ArtistForm artist={artist} profileImageUrl={profileImageUrl} action={updateArtistAction.bind(null, id)} />{!artist.archivedAt ? <form action={archiveArtistAction.bind(null, id)} className="mt-10"><button className="border border-red-600 px-4 py-2 font-black text-red-700">Archive artist</button></form> : null}</main></>;
}
